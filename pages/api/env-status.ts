...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    Object.entries(status).forEach(([key, info]) => {
      ...
      enhancedStatus[key] = { ...info, status: statusType };
    });

    // Add general security metrics
    const securityMetrics = {
      totalKeys: Object.keys(enhancedStatus).length,
      encryptedKeys: Object.values(enhancedStatus).filter((info: any) => info.encrypted).length,
      missingKeys: Object.values(enhancedStatus).filter((info: any) => info.status === 'missing').length,
      exposedKeys: Object.values(enhancedStatus).filter((info: any) => info.status === 'warning').length,
    };
